class UserPanel {
    private navWrapper: HTMLDivElement

    private showUserPanel: boolean = false
    
    private onAnimation: boolean = false

    private userButton: HTMLButtonElement

    private panelBackdrop: HTMLDivElement
    ...
    handleShowUserPanel() {
        ...
        if (this.showUserPanel) {
                      ...
                      <a class="auth-panel-role">
                        <span class="inner-auth-panel-role">
                          <div>
                            ...
                            <div class="auth-role-name">User</div>
                          </div>
                        </span>
                      </a>
            ...
            (this.navWrapper.children[this.navWrapper.childElementCount - 1] as HTMLDivElement).addEventListener("animationend", (event) => {
                if (event.animationName === 'enter') {
                    (this.navWrapper.children[this.navWrapper.childElementCount - 1] as HTMLDivElement).className = "auth-panel-wrapper"

                    this.onAnimation = false;
                }
            })
        } else {
            (this.navWrapper.children[this.navWrapper.childElementCount - 1] as HTMLDivElement).className = "auth-panel-wrapper panel-exit-animation";
...
const userPanel = new UserPanel({
    navWrapper: document.querySelector(".side-navbar") as HTMLDivElement,
    userButton: document.querySelector(".user-button") as HTMLButtonElement,
    panelBackdrop: document.querySelector(".auth-panel-backdrop") as HTMLDivElement
})
